<template>
    <div class="dish-list">
        <DishItem v-for="dish in dishes" :key="dish.id" :dish="dish" @add-to-cart="$emit('add-to-cart', dish)"
            @view-details="$emit('view-details', dish)" @reduce-quantity="$emit('reduce-quantity', dish)" />
    </div>
</template> 
